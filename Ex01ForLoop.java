...
// for 반복문은 비교적 횟수와 관게가 있는 반복문이다.

// while 반복문은 끝이 어딘지 모르는 무언가를 계속 읽어오거나 적을 때 주로 사용이 되지만
// for반복문은 끝이 명확하거나 순서가 있는 뭔가를 다룰 때 주로 사용이 된다.

// for 반복문은 주로 다음과 같은 형태를 가진다.
// for(제어변수 선언과 초기화; 반복 조건식; 제어변수 변화할당식){
//      반복할 코드
//}
...
public class Ex01ForLoop {
    public static void main(String[] args) {
        ...
        // 3. System.out.println("i 의 현재값" + i);
        // 4. i++; i의 현재값: 2
        ...
        // 4. 여러분들이 초기 단게에서 가장 많이 하는 실수는
        //    i 자리에 1을 넣는 실수를 많이 한다.
        //    이러면 무한루프가 되거나 아예 for문이 시작안할 수도 있으니 주의해야한다.

        // 5. i도 하나의 변수이다. 즉 유효범위도 for문 내부인 변수가 되니깐
        //    중복 선언을 조심해야 한다.

        // 6. 다른 제어문들도 마찬가지이지만, for문도 () 뒤에 ; 을 붙이면
        //    반복할 코드 블락이 아에 분리가 되어 의미가 바뀌게 되니 () 뒤에 ; 을 붙이는 것을
        //    주의하자
